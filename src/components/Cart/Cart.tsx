...
export default function Cart() {
  const {user, setPurchaseData, isLoading, isAuthenticated} = useAuth()
  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        if (isLoading) return;
        if (!isAuthenticated) {
          console.log(`url:${user?.url_id}`)
          const response = await api.get('/products/get/to-cart');
          console.log(`товары в корзине с бэка: ${response.data}`)
  ...
  const removeItem = async (slug: string, stat: string) => {
    await api.post('/products/change/status',
      {slug: slug, stat: stat},
